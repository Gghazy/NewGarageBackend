...
public class JwtTokenService(IOptions<JwtOptions> options) : IJwtTokenService
{
    ...
    public (string token, DateTime expiresAt) CreateToken(Guid? userId, string employeeNameAr, string employeeNameEn, string email, IList<Claim>? claims, List<Guid>? branchIds = null)
    {
        ...
        if (branchIds is { Count: > 0 })
        {
            foreach (var branchId in branchIds)
                identityClaims.Add(new Claim("branch", branchId.ToString()));
        }
