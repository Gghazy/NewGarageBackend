...
[Authorize]
public class ClientResourcesController(IMediator _mediator, IStringLocalizer T) : ControllerBase
{
    [HttpPost("pagination")]
    [HasPermission(Permission.ClientResource_Read)]
    public async Task<QueryResult<LookupDto>> GetAll(SearchCriteria search) => await _mediator.Send(new GetAllPaginationQuery<ClientResource>(search));

    [HttpGet]
    public async Task<List<LookupDto>> GetAll()
          => await _mediator.Send(new GetAllLookupQuery<ClientResource>());

    [HttpPost]
    [HasPermission(Permission.ClientResource_Create)]
    public async Task<Guid> Create(LookupRequest req)
        =>await _mediator.Send(new CreateLookupCommand<ClientResource>(req));

    [HttpPut("{id:Guid}")]
    [HasPermission(Permission.ClientResource_Update)]
    public async Task<IActionResult> Update(Guid id, LookupRequest req)
        => await _mediator.Send(new UpdateLookupCommand<ClientResource>(id, req)) ? NoContent() : NotFound();
